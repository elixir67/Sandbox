...
int _tmain(int argc, TCHAR *argv[])
{
    ...
    std::wstring sourcePostfix(dwgPath.substr(dwgPath.find_last_of(L".")+1));
    transform(sourcePostfix.begin(), sourcePostfix.end(), sourcePostfix.begin(), ::tolower);
    if(sourcePostfix.compare(L"dwg") != 0 || !ConvertUtil::File_Exists(dwgPath))
        return EXIT_FAILURE;

    std::wstring sqlitePath(argv[2]);
    size_t pos;
    std::wstring targetDirectory;
    size_t pos1 = sqlitePath.find_last_of(L"/"); 
    size_t pos2 = sqlitePath.find_last_of(L"\\");
    pos = (pos1 > pos2 ? pos1 : pos2);

    if (pos != std::wstring::npos)
    {
        targetDirectory = sqlitePath.substr(0, pos+1);
    }
    else
    { 
        pos = (pos1 < pos2 ? pos1 : pos2);
    ...
    logger.Open(targetDirectory + L"Error.log");
    logger.print("start copying dwg to sqlite");

    std::wstring ProgressLogPath = targetDirectory + L"Progress.log";
    int number = DWGConvertor::ConvertToSqlite(dwgPath, sqlitePath, logger, ProgressLogPath);

    if (number <= 0 )
    {
        logger.log("fail to convert dwg to sqlite, no geometry copied!");
        logger.Close();
        return EXIT_FAILURE;
    }
    char logstr[1024];
    sprintf(logstr, "copy dwg to sqlite successfully, %d geometries has been copied", number);
    logger.print(logstr);
    logger.Close();
